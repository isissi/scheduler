...
describe('Application', () => {
  ...
  it.only('loads data, edits an interview and keeps the spots remaining for Monday the same', async () => {
    ...
    // 4. Change the student name & reselect an interviewer.
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: {
    ...
    // 6. Wait until the element with new student name is shown. 
    await waitForElement(() => getByText(container, 'Lydia Miller-Jones'));

    // 7. Confirm the updated interview has the correct interviewer. 
    expect(getByText(appointment, 'Sylvia Palmer')).toBeInTheDocument();

    // 8. Confirm the spot is still the same. 
    const day = getAllByTestId(container, "day").find(day =>
      queryByText(day, "Monday")
    );
